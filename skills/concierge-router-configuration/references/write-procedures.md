...
4. Optional per-row extras only when asked: `timeout: {minutes, onTimeout: Landing|Url}`, `crmActions: [ConvertLead | Notify{slackChannel?} | AddToCampaign{campaignId, memberStatus} | SalesforceUpdateFields | HubspotUpdateFields | SalesforceUpsertRecord | HubspotUpsertRecord (Concierge only) | SalesforceUpdateOwnership | HubspotUpdateOwnership | SalesforceCreateEvent{relatedTo?, meetingCancellationBehavior?, guestsBehavior?} | HubspotCreateEngagement{relatedTo?, owner?, meetingCancellationBehavior?}]` — field shapes and defaults → api-reference § Write shapes. `Other{kind}` appears only on read (unmodellable node placeholder) and is rejected on write. If a row includes `ConvertLead`, warn the admin in the plan and result: it publishes and fires but does not render in the Flow Builder — they won't see it in the UI (verified 2026-07-30 → api-reference § Write shapes).
5. `catchAll` is **optional** on create and update (CEH-11358) — omitting it on create produces a router with no fallback path; on update it preserves the current catch-all. If unspecified, ask what unmatched submissions should do and state the choice in the plan.
6. Pre-routing steps (`routingSteps`, CEH-11538) only when asked: an ordered list of `{type: "Enrichment", waterfalls: [{dataField, waterfallId}], timeoutSeconds?}` (waterfalls non-empty) and/or `{type: "SpamCheck", writeSpamScoreToDataField}`. Resolve every `waterfallId` via `enrichment-waterfall-list` (match by `name`, take `id`) — never invent one; it is unvalidated on write and a stale id only fails at publish. **Full-replace semantics:** omit `routingSteps` on update to preserve the current steps; supply it to replace the whole steps dimension (`[]` clears); supplied without `routing`, it replaces only the step prefix and keeps the rows. Supplying it on a router whose steps read `Unrepresentable` is rejected 409 → api-reference § Representability.
7. Form changes: each field is `{dataField, label, required, description?, hidden?}` — `dataField` must reference a real, **already-existing** data field (no API lists or creates them: standard defaults like `PersonEmail` are always valid, custom fields need their UUID from the app, and an unknown reference fails the write with 400 → api-reference § Data fields); keep existing fields unless the human asked to change them; the field list must include `PersonEmail`. Trigger changes: `inAppButton` (`[{dataField}]`) and `routerLink` (`[{dataField, label, required?, hidden?}]`) are each writable and replace **only their own kind** — writing one never destroys the others; both must include `PersonEmail`. Branding: `{coverImage?, headingText?, language?}` — merged per sub-field (omitted sub-fields preserved).
...
- `RouterRoutingNotRepresentable` (409) → the update supplied `routingSteps` on a router whose steps contain `Unrepresentable` (CEH-11538): drop `routingSteps` from the payload (a rows-only edit preserves the steps verbatim) or direct the human to edit the steps in the app. Rows-only routing updates stopped returning it with DISTRO-4614.
