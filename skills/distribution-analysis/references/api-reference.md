...
| `distribution-list-put` | Distributions as `{results: [...], total, page, pageSize}` — **iterate `results`**; `total` gives the count (CEH-11548, 2026-09-01; formerly a bare top-level array). Input `{workspaceIds: [...], name?, assignmentType?}`. Each `results[]` item: `{id, published: {distributionId, name, weights: [{userId, weight}], assignmentTypeConfig: {type, handling: {type}}, capping, teamRef: {id}}, state: {userStates: [{userId, type: "Active"\|"Capped"\|"Disabled"\|"Removed"\|"NoLicense", statistics: {assigned, cancelled, noShow, reassignedToThis, reassignedFromThis}}]}}` |
