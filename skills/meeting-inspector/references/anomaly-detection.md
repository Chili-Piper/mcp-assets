...
**Routing fallthrough**  
No routing rule matched this lead's profile — they hit the catch-all or were dropped. Review the `sourceUrl` to see which page triggered the router and whether a rule covers that source.

**Unrouted meeting**  
No concierge-log was found for this meeting. It was likely booked via a direct scheduling link, a handoff, or a manual booking by a rep. Not necessarily a problem, but worth confirming if ownership and CRM write-back behaved correctly.
