...
2. **Find the distribution.** Call `distributionListPut` with `workspaceIds: [<workspace id>]` (and optional `name` filter). The response is a top-level array. From the matching item read:
   ...
   - `state.userStates[]` filtered to `type == "Active"` — the member reps to analyze
...
5. **Build the breakdown and detect imbalance.** Per rep: total, completed, cancelled, no-show, cancel-rate. Then compute the imbalance ratio (top vs. median), compare each rep's actual share to their configured `weight`, and look at day-of-week skew (`dateTime.start`), booking source (read `scheduleOrigin`/`meetingSource` literally), weekly trend, and cancellation actors (from `history[]`). Flag reps with 0 meetings (likely an availability/calendar blocker) and reps whose share diverges sharply from their weight.
